...
- **Draft destination** — where to write the output file. If none is given, write to a temporary file **outside the repository working tree** (e.g. under the system temp directory) with a descriptive name — never into the repo tree, since governance content can contain personnel/organizational data — and report the path.

...
- [`skills/shared/authority-boundaries.md`](skills/shared/authority-boundaries.md) — cross-role authority reference loaded by the four role skills when authority questions span more than one Core Role.
- [`skills/shared/actor-and-role-resolution.md`](skills/shared/actor-and-role-resolution.md) — actor identity + role/circle resolution procedure. Every role skill loads this to figure out who's acting and from which circle/role before producing any output. Defines the prompt preamble for scheduled routines.
- [`skills/shared/triage-gates.md`](skills/shared/triage-gates.md) — the five canonical gates a tension passes through, in the order that makes each worth asking: **already-held authority** (does the sensing role already hold the domain or accountability, or has a role been created since that supersedes this?), role-vs-person (with the blended-tension partition), suggested-meeting-venue annotation (governance vs tactical, for the user's mental model — not an API field), supersession check (S.5.5.1d), and role-attribution policy. Authority is first because on a real 35-tension pass it changed the disposition every time it fired. Loaded by Pattern 3, Pattern 5 (proactive sensing), the `tension-capture` subagent, and the `/holacracy:*` tension commands. See [ADR-0011](docs/adr/0011-separate-tension-readiness-from-tension-disposition.md).
- [`skills/shared/tension-capture-flow.md`](skills/shared/tension-capture-flow.md) — the draft-and-confirm B-flow used to file tensions to a role's GlassFrog backlog via `glassfrog_create_tension(role_id, body)`. Per-tension confirmation; no auto-file; constitutional safeguard preserved.
...
- [`skills/shared/project-review-critics.md`](skills/shared/project-review-critics.md) — the five critic lenses, the finding schema, and the severity/floor/cap/dedupe rules for `/holacracy:review-project`. Seeds both the inline backlog walk and the deep-review subagents.
...
- [`skills/shared/project-disposition-flow.md`](skills/shared/project-disposition-flow.md) — the disposition state machine for GlassFrog projects, adapted from Matt Pocock's engineering triage methodology: `needs-triage` / `needs-info` / `ready-for-agent` / `ready-for-human` / `wontfix`. Answers "who should act next" rather than "is this well-formed" — the project-side analog of the tension readiness/disposition split ([ADR-0011](docs/adr/0011-separate-tension-readiness-from-tension-disposition.md)). Records state in the project's `note` field via a Triage Note convention, since GlassFrog can tag Roles but not Projects; always resolves and surfaces the project's GlassFrog link alongside a reminder to apply the matching tag by hand. Loaded by `/holacracy:project-disposition`.
...
- [`/holacracy:tension-triage`](commands/tension-triage.md) — walk the unprocessed tensions on the actor's roles and decide, per tension, **whether each is ready to be processed**: archive false positives, edit bodies that no longer scan, name authority the role already holds, flag supersession, defer the rest. Sweeps circle-by-circle (18 calls against a real org, versus 81 per-role) because `list_subrole_tensions` is not recursive despite its docs. Readiness only — *disposition* belongs to `/holacracy:process-tension` ([#160](https://github.com/Integral-Productivity/holacracy-claude-plugin/issues/160), not yet shipped). Renamed from `/holacracy:process-inbox`, which survives as a deprecated alias; see [ADR-0011](docs/adr/0011-separate-tension-readiness-from-tension-disposition.md).
...
- [`/holacracy:project-disposition`](commands/project-disposition.md) — classify a single GlassFrog project into Matt Pocock's five-state disposition machine (`needs-triage` / `needs-info` / `ready-for-agent` / `ready-for-human` / `wontfix`). Accepts a GlassFrog project URL, `proj_id`, or name via `$ARGUMENTS`. Writes the disposition to the project's `note` field on explicit per-project confirmation; a `wontfix` disposition can also set `status: "cancelled"` or `"archived"` on a separate confirmation. GlassFrog cannot tag projects via API, so every response ends with the project's link and an explicit reminder to apply the matching tag by hand in the GlassFrog UI.
...
- [`hooks/session-start`](hooks/hooks.json) emits up to two things at session start, fail-silent on any error:
  - **A role-grounding directive** (on wherever it can actually be satisfied). It *demands* that the session resolve and announce its active Holacratic role/circle — per [`skills/shared/actor-and-role-resolution.md`](skills/shared/actor-and-role-resolution.md) — before its first substantive action, and flag any cross-role remit boundary. It is **honest by construction**: the hook has no GlassFrog access at fire time, so the directive requests the grounding and explicitly never claims it happened. Every payload names the plugin version that produced it, and when the hook has nothing to surface it says so in one line rather than emitting nothing — a hook that writes nothing leaves no trace, which is how a stale copy went undetected for two weeks ([#122](https://github.com/Integral-Productivity/holacracy-claude-plugin/issues/122)). **It is emitted only when an authenticated GlassFrog connector is detected** ([#283](https://github.com/Integral-Productivity/holacracy-claude-plugin/issues/283)). The directive tells the session to call `glassfrog_get_me` before its first action; without an authenticated connector — which is every non-interactive session, since OAuth cannot run there — that instruction cannot be carried out, so it is not given. Detection reads the harness's own MCP OAuth store (`${CLAUDE_CONFIG_DIR:-~/.claude}/.credentials.json`) for a glassfrog entry with a non-empty access token: local, no network, no credential prompt. When the connector is not authenticated the hook emits **one informational line** saying the directive was withheld — it never asks a question, because a `SessionStart` payload has no guarantee of a human at the other end. Scope it with env vars: `HOLACRACY_GROUNDING_ASSUME_GLASSFROG=on` forces the capability gate open for deployments whose token lives where this check cannot see it (an OS keychain, a managed store); `HOLACRACY_GROUNDING_EXCLUDE=<regex>` turns it **off** where `$PWD` matches (the normal way to scope it — always-on with named exclusions, so a misconfiguration is visible rather than silent); `HOLACRACY_GROUNDING_DIRECTIVE=off` disables it entirely; `HOLACRACY_GROUNDING_REQUIRE_GLASSFROG=on` and `HOLACRACY_GROUNDING_REQUIRE_PATH=<regex>` are opt-in gates, and exclusion wins over both. (Track A PDCA-1, [#62](https://github.com/Integral-Productivity/holacracy-claude-plugin/issues/62); amended by [ADR-0008](docs/adr/0008-session-injected-honest-grounding-directive.md#amendments).)
...
The markdown in this repo *is* the product, so it is tested like code rather than reviewed like prose.

- [`scripts/skills-lint.sh`](scripts/skills-lint.sh) — six static checks over every shipped file: relative paths resolve from the file citing them, `Step N` citations name headings that exist, frontmatter carries the keys its artifact type needs, a skill's `version:` moved when its content did, referenced slash commands exist (or are declared forward references against an open issue), and no shared reference is orphaned. Deterministic, offline, about a second; gates every PR.
- [`scripts/skills-lint.test.sh`](scripts/skills-lint.test.sh) — the lint's own suite, mutation-checked: for each check, a fixture that violates only that check must fail the lint *and* pass with that check disabled. The second half is what proves a check is load-bearing rather than incidentally covered.
- [`evals/`](evals/README.md) — behavioural evaluation: does a skill actually do what it says? Costs model calls, so it runs nightly rather than per-PR. Golden cases come from a real 47-tension backlog run with recorded correct dispositions.
- [`scripts/run-behavioural-eval.py`](scripts/run-behavioural-eval.py) — the nightly runner. Starts a hermetic session against a synthetic GlassFrog served by [`evals/stub/`](evals/stub/glassfrog_stub.py), once with the plugin loaded and once without, and grades the transcript. Ordering claims — *write the successor before archiving the original*, *no write without a confirmation* — are checked mechanically against the stub's write log rather than judged, so they cost no tokens and cannot drift between nights.
- [`scripts/run-behavioural-eval.test.sh`](scripts/run-behavioural-eval.test.sh) — the runner's own suite, mutation-checked, and it gates every PR even though the runner itself only runs nightly. It substitutes a fake model that replays a scripted plan through the *real* stub, so every path but the model is exercised offline.

The tiering, and why a documentary rule is not an enforced rule, are recorded in [ADR-0012](docs/adr/0012-test-the-skills-not-just-the-scaffolding.md).
